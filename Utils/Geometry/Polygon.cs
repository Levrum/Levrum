...
namespace Levrum.Utils.Geometry
{
    public class LineSegment2 : ICloneable
    {
        ...
        public Point2 PointA = new Point2(0.0, 0.0);
        public Point2 PointB = new Point2(0.0, 0.0);



        /// <summary>
        /// Get the Ax+By=C form of the line equation specified by this line segment.   See reference immediately following.
        /// </summary>
        /// <param name="dA"></param>
        /// <param name="dB"></param>
        /// <param name="dC"></param>
        /// <returns></returns>
        public virtual bool GetAxPlusByEqC(out double dA, out double dB, out double dC)
        {
            dA = dB = dC = 0.0;

            dA = PointB.Y - PointA.Y;
            dB = PointA.X - PointB.X;
            dC = (dA * PointA.X) + (dB * PointA.Y);
            return (true);
        }

        // From http://community.topcoder.com/tc?module=Static&d1=tutorials&d2=geometry2#line_line_intersection  20130601 CDN
        ...
        /// <summary>
        /// Get the intersection point of the lines generated by this line segment and another one.
        /// </summary>
        /// <param name="oOtherSegment"></param>
        /// <returns></returns>
        public virtual Point2 Intersection(LineSegment2 oOtherSegment)
        {
            double A1, B1, C1, A2, B2, C2;
            if (!GetAxPlusByEqC(out A1, out B1, out C1)) { return (null); }
            if (!oOtherSegment.GetAxPlusByEqC(out A2, out B2, out C2)) { return (null); }

            double delta = (A1*B2)-(A2*B1);  // cross product
            if (0.0 == delta) { return (null); }
        ...
        public virtual bool IntersectsWith(LineSegment2 oOtherSegment)
        {
            Point2 intersection = this.Intersection(oOtherSegment);
            if (null == intersection) { return (false); }
            if (intersection == null || 
                intersection.X < Math.Min(PointA.X, PointB.X) ||
                intersection.X > Math.Max(PointA.X, PointB.X) ||
                intersection.X < Math.Min(oOtherSegment.PointA.X, oOtherSegment.PointB.X) ||
                intersection.X > Math.Max(oOtherSegment.PointA.X, oOtherSegment.PointB.X) ||
                intersection.Y < Math.Min(PointA.Y, PointB.Y) ||
                intersection.Y > Math.Max(PointA.Y, PointB.Y) ||
                intersection.Y < Math.Min(oOtherSegment.PointA.Y, oOtherSegment.PointB.Y) ||
                intersection.Y > Math.Max(oOtherSegment.PointA.Y, oOtherSegment.PointB.Y)
                ) {
                return (false);
            }
        ...
        {
            public intersection_helper(string sLabel, double dX, double dY)
            {
                Label = sLabel;
                X = dX;
                Y = dY;
        ...
        /// <summary>
        /// Implementation of ICloneable ... creates a deep copy.
        /// </summary>
        /// <returns></returns>
        public object Clone()
        {
            ...
            LineSegment2 newseg = new LineSegment2(a_new, b_new);
            return (newseg);
        }
    }
